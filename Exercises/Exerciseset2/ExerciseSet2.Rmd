...
1. `r length(which(Variance_Explained_Table[,"Cumulative_percentage_of_explained_variance"]<50))+1`
2. `r round(max(Variance_Explained_Table[,"Eigenvalue"]),2)`
3. The first 10 components captures `r round(Variance_Explained_Table[10,4],2)`% of the original data
...
Variance_Explained_Table_results<-PCA(futures_data[as.Date(rownames(futures_data)) > "2002-11-01" & as.Date(rownames(futures_data)) < "2007-10-01", ], graph=FALSE)
Variance_Explained_Table<-cbind(paste("component",1:ncol(futures_data[as.Date(rownames(futures_data)) > "2007-10-01" & as.Date(rownames(futures_data)) < "2009-03-01", ]),sep=" "),Variance_Explained_Table_results$eig)
Variance_Explained_Table<-as.data.frame(Variance_Explained_Table)
colnames(Variance_Explained_Table)<-c("Component","Eigenvalue", "Percentage_of_explained_variance", "Cumulative_percentage_of_explained_variance")
```
...
4b. Financial Crisis Years:

```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}

Variance_Explained_Table_results<-PCA(futures_data[as.Date(rownames(futures_data)) > "2007-10-01" & as.Date(rownames(futures_data)) < "2009-03-01", ], graph=FALSE)
Variance_Explained_Table<-cbind(paste("component",1:ncol(futures_data[as.Date(rownames(futures_data)) > "2007-10-01" & as.Date(rownames(futures_data)) < "2009-03-01", ]),sep=" "),Variance_Explained_Table_results$eig)
Variance_Explained_Table<-as.data.frame(Variance_Explained_Table)
colnames(Variance_Explained_Table)<-c("Component","Eigenvalue", "Percentage_of_explained_variance", "Cumulative_percentage_of_explained_variance")
```
...
5. During the crisis, the first component (factor of government bonds) became very strong, compared to the pre-crisis period. As such, we can clearly see two moments that influence drasticaly the results. During the crisis, results were much more correlated to the first component
...
MAfreq<-seq(10,250,by=20) collects the data starting from 10 and jumping by 20 units

`r seq(10,250,by=20)`

Then, it loops to check moving averages vs actual data and puts marks on it. Finally, it indicates buy/sell decisions based on the benchmark

2. After some tests, we found out that changing such parameters the returns fluctuate alongside the sharpe ratio (positive correlation), which is something that we would normally expect
